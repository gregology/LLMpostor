...
class TestAutomaticGameFlow:
    ...
    def test_player_disconnect_auto_advance_responding_phase(self, client, mock_content_manager):
        """Test that responding phase auto-advances when disconnected player had already submitted."""
        client2 = socketio.test_client(app)
        client3 = socketio.test_client(app)
        
        # Use unique room name
        room_name = 'test_room_responding_disconnect'
        ...
        try:
            ...
            # Player 3 disconnects (without submitting response)
            client3.disconnect()
            
            # Wait a moment for disconnect handling
            time.sleep(0.1)
            
            # Check that phase advanced to guessing (since remaining players have all submitted)
            game_state = game_manager.get_game_state(room_name)
            assert game_state['phase'] == 'guessing'
            
        finally:
            client2.disconnect()
    
    def test_player_disconnect_auto_advance_guessing_phase(self, client, mock_content_manager):
        """Test that guessing phase auto-advances when disconnected player had already guessed."""
        client2 = socketio.test_client(app)
        client3 = socketio.test_client(app)
        
        # Use unique room name to avoid conflicts
        room_name = 'test_room_guessing_disconnect'
        
        try:
            # Set up game to guessing phase with 3 players
            client.emit('join_room', {
                'room_id': room_name,
                'player_name': 'Player1'
            })
            ...
            # Start round and get to guessing phase
            client.emit('start_round')
            ...
            # All players submit responses
            client.emit('submit_response', {'response': 'Response 1'})
            client2.emit('submit_response', {'response': 'Response 2'})
            client3.emit('submit_response', {'response': 'Response 3'})
            ...
            # Two players submit guesses (Player1 and Player2)
            client.emit('submit_guess', {'guess_index': 1})
            client2.emit('submit_guess', {'guess_index': 2})
            ...
            # Verify that we're still in guessing phase (Player3 hasn't guessed yet)
            game_state = game_manager.get_game_state(room_name)
            assert game_state['phase'] == 'guessing'
            
            # Player 3 disconnects (without submitting guess)
            client3.disconnect()
            
            # Wait a moment for disconnect handling
            time.sleep(0.2)
            
            # Check that phase advanced to results (since remaining players have all guessed)
            game_state = game_manager.get_game_state(room_name)
    ...
    def test_player_disconnect_insufficient_players_reset(self, client, mock_content_manager):
        """Test that game resets to waiting when too few players remain after disconnect."""
        client2 = socketio.test_client(app)
        ...
        try:
            ...
            # One player disconnects, leaving only 1 player
            client2.disconnect()
            ...
            received = client.get_received()
            pause_events = [event for event in received if event['name'] == 'game_paused']
            
            assert len(pause_events) > 0
            pause_event = pause_events[0]
            pause_response = pause_event['args'][0]
            assert pause_response['success'] is False
            assert 'need at least 2 players' in pause_response['error']['message']
