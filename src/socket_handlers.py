"""
Socket.IO event handlers for LLMposter.
"""
...
def register_handlers(socketio, app, room_manager: RoomManager, game_manager: GameManager, content_manager: ContentManager, session_manager: SessionManager, broadcast_manager: BroadcastManager):
    ...
    def handle_disconnect():
        ...
        if session_info:
            ...
            if room_manager.remove_player_from_room(room_id, player_id):
                ...
                broadcast_manager._handle_player_disconnect_game_impact(room_id, player_id)
                
                # Broadcast updated player list to remaining players
                broadcast_manager._broadcast_player_list_update(room_id)
                broadcast_manager._broadcast_room_state_update(room_id)
            
            # Clean up session
            session_manager.delete_session(request.sid)
    ...
    def handle_join_room(data):
        ...
        if session_manager.get_session(request.sid):
            raise ValidationError(
        ...
        try:
            ...
            # Store session info
            session_manager.create_session(request.sid, room_id, player_data['player_id'], player_name)
            ...
            broadcast_manager._send_room_state_to_player(room_id, request.sid)
    ...
    def handle_start_round(data=None):
        ...
        if game_manager.start_new_round(room_id, prompt_dict):
            ...
            broadcast_manager._broadcast_round_started(room_id)
            broadcast_manager._broadcast_room_state_update(room_id)
    ...
    def handle_submit_response(data):
        ...
        if game_manager.submit_player_response(room_id, player_id, response_text):
            ...
            if updated_game_state and updated_game_state['phase'] == 'guessing':
                ...
                broadcast_manager._broadcast_guessing_phase_started(room_id)
            else:
                # Broadcast response count update to all players (without revealing content)
                broadcast_manager._broadcast_response_submitted(room_id)
    ...
    def handle_submit_guess(data):
        ...
        if game_manager.submit_player_guess(room_id, player_id, actual_response_index):
            ...
            if updated_game_state and updated_game_state['phase'] == 'results':
                ...
                broadcast_manager._broadcast_results_phase_started(room_id)
            else:
                # Broadcast guess count update to all players (without revealing content)
                broadcast_manager._broadcast_guess_submitted(room_id)
